...
function loadIdioms() {
  try {
    ...
    return notes.filter(n => n.type === 'Idiom');
  } catch {
    return [];
  }
}

export default function Idioms({ onBack }) {
  const cards = loadIdioms();
    ...
    return (
      <div className="vocab-page">
        ...
        <div style={{ textAlign: 'center', marginTop: 80 }}>
          ...
          <h2 style={{ margin: '16px 0 8px' }}>No Idioms yet!</h2>
          <p style={{ color: '#718096' }}>Go to My Notes and import the all-notes-fixed.json file to get 100+ idioms.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="vocab-page">
      ...
      <div className="progress-bar">
        <div className="progress-fill" style={{ width: `${((index + 1) / total) * 100}%`, background: '#d69e2e' }} />
      </div>
      <div className={`flashcard ${flipped ? 'flipped' : ''}`} onClick={() => setFlipped(!flipped)}>
        <div className="flashcard-inner">
          <div className="flashcard-front">
            <span className="card-type">Idiom</span>
            <h1 className="card-word">{card.word}</h1>
            <p className="tap-hint">Tap to reveal meaning</p>
          </div>
          <div className="flashcard-back" style={{ background: '#d69e2e' }}>
            <span className="card-type">Idiom</span>
